...
export const FLOWS = [
...

  {
    ...
    steps: [
      ...
      },
      {
        ...
        body: 'Gameplay events (WAGER / CASINO_WIN / DEPOSIT_MADE / LOGIN) hit gamru. The mission engine finds ACTIVE missions whose objective matches the event, checks sub-conditions, and adds the delta to the player’s progress row for the current period (daily/weekly/monthly/lifetime). When all objectives hit target the mission becomes COMPLETED.',
        endpoints: ['gamru-integration-events'],
      },
      {
        title: 'Player claims the chest',
        body: 'On a COMPLETED mission the player taps claim. The platform proxies to gamru, which applies the reward (XP via the rank engine, tokens, or records bonus_cash/free_spins), writes a player_rewards row + audit log, and flips the mission to CLAIMED.',
        endpoints: ['games-missions-claim', 'gamru-players-mission-claim'],
      },
    ],
    sequence: [
      'Operator -> Gamru: POST /api/gamification/missions/add (ACTIVE)',
      'Player -> Games: GET /api/missions (snapshot from gamru)',
      'Player -> Games: POST /api/missions/:id/join',
      'Player -> Games: POST /api/activity (play)  ──▶  Gamru: POST /api/integration/events (WAGER…)',
      'Gamru -> Gamru: match mission, +delta to player_mission_progress, COMPLETED at target',
      'Player -> Games: POST /api/missions/:id/claim',
      'Games -> Gamru: POST /api/players/:id/missions/:missionId/claim',
...

  {
    ...
    steps: [
      ...
      },
      {
        ...
        body: 'Missions inside a bundle are an independent track (no per-bucket exclusivity). They progress from the same gameplay events as standalone missions.',
        endpoints: ['games-activity', 'gamru-integration-events'],
      },
      {
        title: 'Player claims per-mission chests',
        body: 'Each completed mission in the bundle is claimed individually; the bundle’s completed/total recomputes.',
        endpoints: ['games-bundles-claim', 'gamru-players-mission-claim'],
      },
    ],
    sequence: [
...

  {
    ...
    tag: 'Competition',
    intro:
      'Operators run time-boxed tournaments in gamru. The casino submits player scores; gamru keeps the authoritative standings and awards prizes.',
    actors: ['Operator', 'Player', 'Games platform', 'Gamru engine'],
    steps: [
      {
        title: 'Operator creates the tournament',
        body: 'Created as a gamification item (feature tournaments) with industry, eligible games and a schedule.',
        endpoints: ['gamru-gam-add'],
      },
      {
        title: 'Player joins the race',
        body: 'The casino lists active tournaments from the player snapshot and shows state (SCHEDULED / IN_PROGRESS / ENDED).',
        endpoints: ['games-tournaments-list'],
      },
      {
        title: 'Platform submits scores',
        body: 'As the player earns points the casino posts them; the platform proxies to gamru’s leaderboard endpoint (idempotent on email+tournamentId).',
        endpoints: ['games-tournaments-score', 'gamru-tlb-score'],
      },
      {
        title: 'Standings & prizes',
        body: 'gamru ranks participants. Operators view standings in the back office; prizes are awarded per the tournament config.',
        endpoints: ['gamru-tlb-get'],
      },
    ],
    sequence: [
      'Operator -> Gamru: POST /api/gamification/tournaments/add',
      'Player -> Games: GET /api/tournaments',
      'Player -> Games: POST /api/tournaments/:id/score { points }',
      'Games -> Gamru: POST /api/tournament-leaderboard/:id/score { email, points }',
      'Gamru --> Games: { rank }',
    ],
    notes: ['Scores are idempotent on (email, tournamentId) — re-submitting the same total is safe.'],
  },
